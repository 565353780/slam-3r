...
from slam_threer.Method.misc import invalid_to_zeros, invalid_to_nans
...
def depthmap_to_absolute_camera_coordinates(depthmap, camera_intrinsics, camera_pose, dataset, **kw):
    """
    Args:
        - depthmap (HxW array):
        - camera_intrinsics: a 3x3 matrix
    ...
    Returns:
        pointmap of absolute coordinates (HxWx3 array), and a mask specifying valid pixels."""
    if dataset == "Aria":
        X_cam, valid_mask = depthmap_to_camera_coordinates_ARIA(depthmap, camera_intrinsics)
    else:
...
def depthmap_to_world_and_camera_coordinates(depthmap, camera_intrinsics, camera_pose, dataset, **kw):
    """
    Args:
        - depthmap (HxW array):
        - camera_intrinsics: a 3x3 matrix
...
def multiview_normalize_pointcloud(pts:list, norm_mode='avg_dis', valid=None):
    """ renorm pointmaps pts1, pts2 with norm_mode
    """
    ...
    if norm_mode == 'avg':
        ...
        all_pts = []
        for i in range(len(pts)):
            nan_pts = invalid_to_nans(pts[i], valid[i], ndim=3)
            all_pts.append(nan_pts)
        all_pts = torch.cat(all_pts, dim=1)
    ...
    res = [pts1 / norm_factor for pts1 in pts]
